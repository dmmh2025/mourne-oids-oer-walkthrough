    ...
    (async () => {
      ...
      try {
        ...
        const sections = toArray<any>(data.sections).map((s) => ({
          ...s,
          items: toArray<any>(s?.items).map((i) => ({ ...i, photos: toArray<string>(i?.photos) })),
        }));
        ...
        const normalized: Submission = {
          ...
          predicted:
            typeof data.predicted === "number"
              ? data.predicted
              : (Number(data.section_total) || 0) + (Number(data.service_total) || 0),
          sections,
        };
  ...
  return (
    <>
      <main style={{ maxWidth: 1000, margin: "0 auto", padding: 16 }}>
        ...
        {sub && (
          <article style={{ border: "1px solid #e5e7eb", borderRadius: 12, background: "white", padding: 12 }}>
            <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "baseline" }}>
                <strong style={{ fontSize: 18 }}>
                  {sub.store || "Unknown"} — {sub.user_name || "Anon"}
                </strong>
                <span style={{ color: "#6b7280" }}>{new Date(sub.created_at).toLocaleString()}</span>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <Badge label="Stars" value={`${"★".repeat(starsForPercent(sub.predicted || 0))}${"☆".repeat(5 - starsForPercent(sub.predicted || 0))}`} />
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {toArray<SectionPayload>(sub.sections).map((sec, sidx) => {
                ...
                return (
                  <section key={`${title}-${sidx}`} style={{ border: "1px solid #eef2f7", borderRadius: 10 }}>
                    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderBottom: "1px solid #eef2f7", background: "#f8fafc", borderRadius: "10px 10px 0 0" }}>
                      <strong>{title}</strong>
                      <small style={{ color: "#64748b" }}>Max {max} • {mode}</small>
                    </header>
                    <div style={{ padding: 10, display: "grid", gap: 8 }}>
                      {toArray<Item>(sec.items).map((it, iidx) => {
                        ...
                        return (
                          <div key={`${label}-${iidx}`} style={{ border: "1px solid #f1f5f9", borderRadius: 10, padding: 10, background: "#fff" }}>
                            <div style={{ display: "flex", gap: 8, alignItems: "flex-start", flexWrap: "wrap" }}>
                              <span title={it.checked ? "Checked" : "Not checked"}>{it.checked ? "✅" : "⬜️"}</span>
                              <div style={{ fontWeight: 600 }}>{label}{ptsText}</div>
                            </div>
                            {photos.length > 0 && (
                              <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap" }}>
                                {photos.map((url, pidx) => (
                                  <button
                                    ...
                                    style={{ border: "1px solid #e5e7eb", padding: 0, borderRadius: 8, overflow: "hidden", cursor: "pointer", background: "transparent" }}
                                    title="Click to view"
                                  >
                                    <img src={url} alt="attachment" style={{ width: 96, height: 96, objectFit: "cover", display: "block" }} />
                                  </button>
                                ))}
                              </div>
                            )}
      ...
      {lightbox && (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "grid", placeItems: "center", zIndex: 1000, padding: 16 }}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ position: "relative", maxWidth: "95vw", maxHeight: "90vh", borderRadius: 12, overflow: "hidden", background: "#000", boxShadow: "0 10px 30px rgba(0,0,0,.4)" }}
          >
            <img src={lightbox} alt="full" style={{ maxWidth: "95vw", maxHeight: "90vh", objectFit: "contain", display: "block" }} />
            <button
              type="button"
              onClick={() => setLightbox(null)}
              style={{ position: "absolute", top: 8, right: 8, padding: "8px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "white", fontWeight: 800, cursor: "pointer" }}
            >
              ✕ Close
            </button>
          </div>
...
function Badge(props: { label: string; value: string }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 999, background: "#f1f5f9", border: "1px solid #e5e7eb", color: "#111827", fontWeight: 700 }}>
