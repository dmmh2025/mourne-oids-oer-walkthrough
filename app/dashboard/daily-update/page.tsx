  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-100 to-indigo-200 px-4 py-10">
      <div className="mx-auto w-full max-w-5xl rounded-2xl border border-white/50 bg-white/65 p-5 shadow-2xl backdrop-blur-md sm:p-8">
        <div className="overflow-hidden rounded-xl border border-slate-200/60">"use client";
      ...
      <div className="shell">
        ...
        <header className="header">
          ...
          <p className="subtitle">
            {dates ? (
              <>
                Date: <b>{dates.today}</b> • Week start: <b>{dates.week_start}</b> (Mon)
              </>
            ) : (
              "Loading dates…"
            )}
                ...
                <div key={b.store} className="storeCard">
                  ...
                  <div className="cols">
                    ...
                    <div className="col">
                      ...
                      <div className="metricRow">
                        <span>Service • Yesterday (Mgr)</span>
                        <b>{b.serviceY.opening || b.serviceY.closing ? `${b.serviceY.opening || "—"} / ${b.serviceY.closing || "—"}` : "—"}</b>
                      </div>
                      <div className="metricRow">
                        <span>Service • DOT</span>
                        <b className={pill(dotTone(b.serviceY.dot))}>{fmtPct(b.serviceY.dot, 1)}</b>
                      </div>
                      <div className="metricRow">
                        <span>Service • Extremes &gt;40</span>
                        <b className={pill(extremesTone(b.serviceY.ext))}>{fmtPct(b.serviceY.ext, 2)}</b>
                      </div>
                      <div className="metricRow">
                        <span>Service • R&amp;L</span>
                        <b className={pill("neutral")}>{fmtMinutes(b.serviceY.rnl, 1)}</b>
                      </div>
                      <div className="metricRow">
                        <span>Service • Add Hours (Yday)</span>
                      ...
                      <div className="metricRow">
                        <span>OSA • Latest (WTD)</span>
                        <b className={pill("neutral")}>
                          {b.osa.latest?.stars != null ? `${b.osa.latest.stars}★` : "—"}
                          {b.osa.latest?.is_elite ? " • Elite" : ""}
                        </b>
                      </div>
                    ...
                    <div className="col">
                      ...
                      <div className="targets">
                        <div className="tRow"><span>Load</span><b>{b.inputs?.target_load_time_mins != null ? `${b.inputs.target_load_time_mins}m` : "—"}</b></div>
                        <div className="tRow"><span>Rack</span><b>{b.inputs?.target_rack_time_mins != null ? `${b.inputs.target_rack_time_mins}m` : "—"}</b></div>
                        <div className="tRow"><span>ADT</span><b>{b.inputs?.target_adt_mins != null ? `${b.inputs.target_adt_mins}m` : "—"}</b></div>
                        <div className="tRow"><span>Extremes &gt;40</span><b>{b.inputs?.target_extremes_over40_pct != null ? `${b.inputs.target_extremes_over40_pct}%` : "—"}</b></div>
                      </div>
                  ...
                  <tbody>
                    {osaYesterday.map((r) => (
                      <tr key={r.id}>
                        ...
                        <td style={{ textAlign: "right" }}>{r.stars ?? "—"}</td>
                        <td style={{ textAlign: "right" }}>{r.points_lost ?? "—"}</td>
                        <td>{r.is_elite ? "Yes" : "—"}</td>
